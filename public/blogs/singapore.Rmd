...
Even though the adj R square improved a lot by adding room_type, it is still only 0.45(<0.5). I want to further investigate the significance of the number of bathrooms, beds, bedrooms and the size of the house. 
```{r, fig.height = 10, fig.width=10}
...
vif(model1)
```
After adding these variables, adjusted R-squared increases to 0.56. This model now is able to explain 56% of the variance of `price_4_night`. However, I note that the P value of the East Region neighbourhood is not significant. As a consequence of this finding I adapt the model further by adapting the neighbourhood value. 
...
I then select the more significant neighbourhoods to include in my model based on the summaries from before. The cleaned model is shown below.
```{r}
...
Now I will split the data into a test and train segments to evaluate the usefulness and accuracy of the model I built above as a means to predict AirBnB prices in Singapore.
...
Next I generate predictions from the model and calculate the Root Mean Square Error (RMSE), which is the standard deviation between my predicted points and the actual values.
...
The RMSE is high, suggesting that my model is imperfect. However this is understandable given the pitfalls in the original data as well as the weak correlation between my dependant variables and independent variable first seen in my correlation plot. 
...
From my summary table, I can say most of my P values are significant while maintaining R square in a relatively high number.
